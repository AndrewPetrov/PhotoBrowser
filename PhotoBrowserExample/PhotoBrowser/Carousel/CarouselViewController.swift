...
class CarouselViewController: UIViewController, Presentable {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        imageViewOnTopOfCollectionView.image = modelInputOutput.item(
    ...
    private func setupNavigationBar() {
        let allMediaBarButtonItem = UIBarButtonItem(
            title: "AllMedia",
            style: .plain,
            target: self,
            action: #selector(switchToContainerPresentation)
        )
    ...
    private func updateTitleView() {
        ...
        titleView?.setup(sender: modelInputOutput.senderName(), info: info)
    }
    ...
    @IBAction private func actionButtonDidTap(_ sender: Any) {
        ...
        let setAsProfilePictureAction = UIAlertAction(
    ...
    private func cacheFirstImagesForCarouselViewAdapter() {
        ...
        if presentationInputOutput.currentItemIndex(withTypes: supportedTypes).row < modelInputOutput.numberOfItems(
            withTypes: supportedTypes
        ) {
        ...
        for delta in start..<count {
            ...
            let safeIndex = min(
                max(currentIndexPath.row + delta, 0),
                modelInputOutput.numberOfItems(withTypes: supportedTypes) - 1
            )
